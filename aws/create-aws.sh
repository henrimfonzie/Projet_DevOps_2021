...
# Définition des Variables pour la creation RDS MySQL
Class="db.t2.micro"
Identifier="SGBD_TEST"
Engine="mysql"
Ava_Zone="us-east-1b"
DbName="Equipe_1_DB_TEST"
...
VPC_DEF_ID=$(aws ec2 create-default-vpc \
  --query 'Vpc.{VpcId:VpcId}' \
  --output text )

aws ec2 create-tags --resources $VPC_DEF_ID --tags Key=Name,Value="VPC Par Defaut Equipe_1"
RDS_SubNet_DEF=$(aws ec2 create-default-subnet --availability-zone us-east-1b)


echo "  VPC ID Default '$VPC_DEF_ID' CREATED in '$AWS_REGION' region."
echo "Le DBSunet a ete crée avec l ID: " $RDS_SubNet_DEF

echo "------- "
echo ID du VPC par defaut
echo $VPC_DEF_ID
echo "--------"
sleep 5
DbSecGrpID=$(aws ec2 create-security-group --group-name RDSSecGrp \
            --description "Security Group for RDS" \
            --vpc-id "$VPC_DEF_ID" \
            --output text)

echo "------- "
echo IDDbSecGrpID du RDS
echo $DbSecGrpID
echo "------- "

DbSubnetID_1=$(aws ec2 create-subnet --vpc-id "$VPC_DEF_ID" --cidr-block 172.31.16.0/24 --availability-zone us-east-1b --query 'Subnet.SubnetId' --output text)
DbSubnetID_2=$(aws ec2 create-subnet --vpc-id "$VPC_DEF_ID" --cidr-block 172.31.32.0/24 --availability-zone us-east-1c --query 'Subnet.SubnetId' --output text)
echo " "
echo "------- "
echo ID des 2 Subnet RDS
echo $DbSubnetID_1
echo $DbSubnetID_2
echo "------- "
#aws rds create-tags --resources "$DbSubnetID_1" --tags 'Key=Name,Value=GRP1-us-east_1b-DB-Subnet'
#aws rds create-tags --resources "$DbSubnetID_2" --tags 'Key=Name,Value=GRP1-us-east_1c-DB-Subnet'
...
echo ID du Subnet Group RDS²
echo $DBSUBNET_GROUP
echo "------- "
aws ec2 create-tags --resources "$DbSubnetID_1" --tags 'Key=Name,Value=GRP1-us-east_1b-DB-Subnet'


#aws rds create-db-instance --allocated-storage 5 --db-instance-class $Class --db-instance-identifier "Test-RDS" --engine "mysql" --availability-zone us-east-1b --db-subsubnet" --port 3306 --db-name projet --master-username admin --master-user-password Password123 --vpc-security-group-ids "sg-0c32f78b9720c6135"
#aws rds create-db-instance --allocated-storage 5 --db-instance-class $Class --db-instance-identifier "Test-RDS" --engine "mysql" --availability-zone "us-east-1b" --db-subsubnet $DBSUBNET_GROUP --port 3306 --db-name projet --master-username admin --master-user-password Password123 

aws rds create-db-instance --allocated-storage 5 --db-instance-class $Class --db-instance-identifier "Test-RDS" --engine "mysql" --availability-zone us-east-1b --port 3306 --db-name projet --master-username admin --master-user-password Password123
